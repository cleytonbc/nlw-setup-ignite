...
export function HabitDay({ amountOfHabits = 0, amountCompleted = 0, date, ...rest }: Props) {
  ...
  const isCurrenDay = dayjs(date).isSame(today)

  return (
    <TouchableOpacity
      className={clsx("rounded-lg border-2 m-1", {
        'bg-zinc-900 border-zinc-800': amountAccomplishedPercentage === 0,
        'bg-violet-800 border-violet-800': amountAccomplishedPercentage > 0 && amountAccomplishedPercentage < 20,
        ...
        'border-white border-4': isCurrenDay
      })}
